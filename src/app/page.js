...
function TypingDots() {
  return (
    <div style={{ display: "flex", gap: 5, padding: "14px 20px", alignItems: "center" }}>
      ...
      <span style={{ fontSize:11, color:"rgba(255,255,255,.12)", marginLeft:8, fontFamily:"var(--ff-m)" }}>thinking...</span>
    </div>
  );
}
...
function ChatBubble({ role, content }) {
  ...
  return (
    <div style={{ display:"flex", gap:10, padding:"5px 0", flexDirection:isBot?"row":"row-reverse", alignItems:"flex-start", animation:"ffSlide .3s ease-out" }}>
      {isBot && <div style={{ width:32, height:32, minWidth:32, borderRadius:9, background:"linear-gradient(135deg,#E8553A,#BE185D)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:800, color:"#fff", fontFamily:"var(--ff-d)", boxShadow:"0 2px 8px rgba(232,85,58,.25)", marginTop:2 }}>F</div>}
      <div style={{ padding:"12px 16px", borderRadius:14, borderTopLeftRadius:isBot?3:14, borderTopRightRadius:isBot?14:3, background:isBot?"rgba(255,255,255,.04)":"rgba(232,85,58,.1)", border:`1px solid ${isBot?"rgba(255,255,255,.06)":"rgba(232,85,58,.15)"}`, fontSize:14, lineHeight:1.75, color:"rgba(255,255,255,.85)", fontFamily:"var(--ff-b)", whiteSpace:"pre-wrap", maxWidth:"85%" }}>{content}</div>
    </div>
  );
}
    ...
    <div style={{ flex:1, overflowY:"auto" }}>
      <div style={{ padding:"10px 16px", borderBottom:"1px solid rgba(255,255,255,.04)" }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <span style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,.2)", fontFamily:"var(--ff-m)" }}>{pct}%</span>
        </div>
      </div>
      {steps.map((s, si) => {
        const sd = ct[s.id] || 0;
          ...
          <div key={s.id}>
            <div onClick={() => onNav(si, Math.min(sd, s.tasks.length-1))} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", cursor:"pointer", background:isActive?"rgba(255,255,255,.025)":"transparent", borderLeft:`3px solid ${isActive?s.color:stepDone?s.color+"50":"transparent"}` }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ fontSize:9, fontWeight:700, letterSpacing:".1em", color:isActive?s.color:"rgba(255,255,255,.2)", fontFamily:"var(--ff-m)", textTransform:"uppercase" }}>STEP {s.id} · {sd}/{s.tasks.length}</div>
                <div style={{ fontSize:13, color:isActive?"rgba(255,255,255,.85)":"rgba(255,255,255,.4)", fontFamily:"var(--ff-d)", fontWeight:isActive?700:400, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{s.title}</div>
              </div>
            </div>
                ...
                return (
                  <div key={t.id} style={{ position:"relative", paddingLeft:16, paddingBottom:1 }}>
                    ...
                    <div onClick={() => onNav(si, ti)} style={{ padding:"4px 0", cursor:"pointer" }}>
                      <div style={{ fontSize:11, fontWeight:curr?600:400, color:tdone?"rgba(255,255,255,.45)":curr?"rgba(255,255,255,.75)":"rgba(255,255,255,.14)", fontFamily:"var(--ff-m)" }}>{t.title}</div>
                      {tdone && d && <div style={{ marginTop:3, padding:"5px 8px", borderRadius:5, background:"rgba(255,255,255,.018)", border:"1px solid rgba(255,255,255,.03)", fontSize:10.5, lineHeight:1.45, color:"rgba(255,255,255,.28)", fontFamily:"var(--ff-b)" }}>{d.length > 150 ? d.slice(0,150)+"…" : d}</div>}
                    </div>
                  </div>
                );
              })}
  ...
  return (
    <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:32, position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ textAlign:"center", maxWidth:460, position:"relative", zIndex:1 }}>
        <div style={{ width:60, height:60, borderRadius:16, margin:"0 auto 24px", background:"linear-gradient(135deg,#E8553A,#BE185D)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, fontWeight:800, color:"#fff", fontFamily:"var(--ff-d)", boxShadow:"0 8px 40px rgba(232,85,58,.3)" }}>F</div>
        <div style={{ fontSize:10, fontWeight:700, letterSpacing:".3em", color:"rgba(255,255,255,.18)", marginBottom:14, fontFamily:"var(--ff-m)" }}>AI STARTUP MENTOR</div>
        <h1 style={{ fontSize:"clamp(34px,5vw,48px)", fontWeight:400, lineHeight:1.1, margin:"0 0 14px", fontFamily:"var(--ff-d)", letterSpacing:"-.02em" }}>Founder<span style={{ color:"#E8553A" }}>Forge</span></h1>
        ...
        <button onClick={() => signIn("google")} style={{ padding:"14px 32px", borderRadius:10, border:"1px solid rgba(255,255,255,.12)", background:"rgba(255,255,255,.04)", color:"#fff", fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"var(--ff-b)", display:"inline-flex", alignItems:"center", gap:10, transition:"all .2s" }}
        ...
        <div style={{ marginTop:40, display:"flex", gap:6, justifyContent:"center", flexWrap:"wrap" }}>
          {CURRICULUM.map(s => <span key={s.id} style={{ fontSize:9, padding:"3px 8px", borderRadius:4, background:"rgba(255,255,255,.02)", border:"1px solid rgba(255,255,255,.03)", color:"rgba(255,255,255,.18)", fontFamily:"var(--ff-m)" }}>{s.icon} {s.title}</span>)}
        </div>
      </div>
    </div>
  );
    ...
    return (
      <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", padding:32, position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ textAlign:"center", maxWidth:460, position:"relative", zIndex:1 }}>
          ...
          <h1 style={{ fontSize:32, fontFamily:"var(--ff-d)", margin:"0 0 24px" }}>Start your first project</h1>
          {showNewForm ? (
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:10 }}>
              <input value={newName} onChange={e => setNewName(e.target.value)} autoFocus
                ...
                style={{ width:280, padding:"12px 18px", borderRadius:10, textAlign:"center", border:"1px solid rgba(255,255,255,.15)", background:"rgba(255,255,255,.04)", color:"#fff", fontSize:14, outline:"none", fontFamily:"var(--ff-b)" }} />
              <button onClick={() => { if (newName.trim()) { createProject(newName.trim()); setNewName(""); setShowNewForm(false); } }} disabled={!newName.trim()}
                style={{ padding:"10px 28px", borderRadius:8, border:"none", background:newName.trim()?"linear-gradient(135deg,#E8553A,#BE185D)":"rgba(255,255,255,.05)", color:newName.trim()?"#fff":"rgba(255,255,255,.2)", fontSize:12, fontWeight:700, cursor:newName.trim()?"pointer":"not-allowed", fontFamily:"var(--ff-m)" }}>BEGIN →</button>
            </div>
          ) : (
            <button onClick={() => setShowNewForm(true)} style={{ padding:"14px 36px", borderRadius:10, border:"none", background:"linear-gradient(135deg,#E8553A,#BE185D)", color:"#fff", fontSize:13, fontWeight:700, cursor:"pointer", fontFamily:"var(--ff-m)", boxShadow:"0 4px 30px rgba(232,85,58,.3)" }}>NEW PROJECT →</button>
          )}
        </div>
      </div>
    );
      ...
      <div style={{ width:280, minWidth:280, height:"100vh", background:"rgba(255,255,255,.012)", borderRight:"1px solid rgba(255,255,255,.05)", display:"flex", flexDirection:"column" }}>
        <div style={{ padding:"14px 16px", borderBottom:"1px solid rgba(255,255,255,.05)" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
            <div style={{ fontSize:18, fontFamily:"var(--ff-d)" }}>Founder<span style={{ color:"#E8553A" }}>Forge</span></div>
            <button onClick={() => signOut()} style={{ fontSize:10, padding:"4px 8px", borderRadius:4, border:"1px solid rgba(255,255,255,.08)", background:"transparent", color:"rgba(255,255,255,.25)", cursor:"pointer", fontFamily:"var(--ff-m)" }}>Sign out</button>
          </div>
          <div style={{ fontSize:11, color:"rgba(255,255,255,.3)", marginBottom:8, fontFamily:"var(--ff-b)" }}>{session.user.name}</div>
          <div style={{ fontSize:9, fontWeight:700, letterSpacing:".1em", color:"rgba(255,255,255,.15)", fontFamily:"var(--ff-m)", marginBottom:6 }}>PROJECTS</div>
          {projects.map(p => (
            <div key={p.id} onClick={() => setActiveId(p.id)} style={{ padding:"6px 10px", borderRadius:6, cursor:"pointer", marginBottom:2, background:p.id===activeId?"rgba(255,255,255,.05)":"transparent", border:p.id===activeId?"1px solid rgba(255,255,255,.08)":"1px solid transparent", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
              <span style={{ fontSize:12, color:p.id===activeId?"rgba(255,255,255,.9)":"rgba(255,255,255,.4)", fontFamily:"var(--ff-b)", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{p.name}</span>
          ...
          <button onClick={() => { const n = "Project " + (projects.length+1); createProject(n); }}
            style={{ width:"100%", padding:"6px", borderRadius:5, border:"1px dashed rgba(255,255,255,.08)", background:"transparent", color:"rgba(255,255,255,.2)", fontSize:10, cursor:"pointer", fontFamily:"var(--ff-m)", marginTop:4 }}>+ New Project</button>
        </div>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", height:"100vh", minWidth:0 }}>
        <div style={{ padding:"10px 20px", borderBottom:"1px solid rgba(255,255,255,.05)", background:"rgba(255,255,255,.012)", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div>
              <div style={{ fontSize:9, fontWeight:700, letterSpacing:".11em", color:step.color, fontFamily:"var(--ff-m)", textTransform:"uppercase" }}>STEP {step.id} · TASK {taskIdx+1}/{step.tasks.length}{isRevisiting?" · REVISITING":""}</div>
              <div style={{ fontSize:14, fontFamily:"var(--ff-d)", color:"rgba(255,255,255,.9)" }}>{task.title}</div>
            </div>
          </div>
        </div>
        <div style={{ padding:"8px 20px", borderBottom:"1px solid rgba(255,255,255,.03)" }}>
          <div style={{ maxWidth:660, margin:"0 auto", display:"flex", gap:6, flexWrap:"wrap" }}>
            <div style={{ flex:1, minWidth:180, padding:"6px 10px", borderRadius:6, background:`${step.color}06`, border:`1px solid ${step.color}10`, display:"flex", alignItems:"center", gap:6 }}>
              <span style={{ fontSize:8, fontWeight:700, letterSpacing:".1em", padding:"2px 5px", borderRadius:3, background:`${step.color}15`, color:step.color, fontFamily:"var(--ff-m)", textTransform:"uppercase" }}>GOAL</span>
              <span style={{ fontSize:11, color:"rgba(255,255,255,.4)", fontFamily:"var(--ff-b)", lineHeight:1.3 }}>{task.goal}</span>
            </div>
            <div style={{ flex:1, minWidth:180, padding:"6px 10px", borderRadius:6, background:"rgba(255,255,255,.015)", border:"1px solid rgba(255,255,255,.03)", display:"flex", alignItems:"center", gap:6 }}>
              <span style={{ fontSize:8, fontWeight:700, letterSpacing:".1em", padding:"2px 5px", borderRadius:3, background:"rgba(255,255,255,.05)", color:"rgba(255,255,255,.25)", fontFamily:"var(--ff-m)", textTransform:"uppercase" }}>OUTPUT</span>
              <span style={{ fontSize:11, color:"rgba(255,255,255,.25)", fontFamily:"var(--ff-b)", lineHeight:1.3 }}>{task.output}</span>
            </div>
          </div>
        </div>
